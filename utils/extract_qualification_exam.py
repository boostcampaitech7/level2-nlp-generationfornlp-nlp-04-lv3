...
problem_pdf_path = '/data/ephemeral/home/ingue/level2-nlp-generationfornlp-nlp-04-lv3/utils/new_exam_data/2022년_제1회_고등_한국사_문제.pdf' # 문제 경로
answer_pdf_path  = '/data/ephemeral/home/ingue/level2-nlp-generationfornlp-nlp-04-lv3/utils/new_exam_data/2022년_제1회_고등_정답(확정안).pdf'
...
def extract_problems_from_pdf(pdf_path):
    ...
    for page_number in range(len(doc)):
        ...
        for match in matches:
            ...
            # 반복 지문 확인
            repeated_match = re.search(r'\[\d+～\d+\](.+)물음에 답하시오\.', question_text)
            if repeated_match:
                common_paragraph = repeated_match.group(1).strip()

            indices0 = next((index for index, item in enumerate(question_parts) if '것은?' in item), -1)
            indices1 = next((index for index, item in enumerate(question_parts) if '①' in item), -1)
            indices2 = next((index for index, item in enumerate(question_parts) if '②' in item), -1)
            indices3 = next((index for index, item in enumerate(question_parts) if '③' in item), -1)
            indices4 = next((index for index, item in enumerate(question_parts) if '④' in item), -1)
            passage = question_parts[0].strip()
            question = {'question': passage, 'choices': [''.join(question_parts[indices1:indices2])[1:], ''.join(question_parts[indices2:indices3])[1:], ''.join(question_parts[indices3:indices4])[1:], ''.join(question_parts[indices4:])[1:]]}
