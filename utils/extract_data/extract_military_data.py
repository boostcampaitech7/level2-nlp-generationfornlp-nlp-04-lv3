...
class MilitaryKoreanDataExtractor:
    ...
    def replace_unwanted_sentences(text):
        text = re.sub(
            r"다음은 수업 중 학생의 발표이다\. 물음에 답하시오\.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"다음은 강연의 일부이다\. 물음에 답하시오\.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"(가)는 학교에 게시할 안내문을 작성하기 위한 학생회 학생들의 회의이고, (나)는 이를 바탕으로 작\n성한 글의 초고이다\. 물음에 답하시오\.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"(가)는 학생들의 토의이고, (나)는 이를 바탕으로 작성한 글의 초고이다. 물음에 답하시오.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"다음은 협상의 일부이다. 물음에 답하시오.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"다음은 학생이 수업 시간에 한 발표이다. 물음에 답하시오.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        text = re.sub(
            r"다음을 읽고 물음에 답하시오.", "다음 글을 읽고 물음에 답하시오.", text
        )
        text = re.sub(
            r"다음은 학급 임원 3명이 한 회의의 일부이다. 물음에 답하시오.",
            "다음 글을 읽고 물음에 답하시오.",
            text,
        )
        return text
    ...
    def extract_paragraphs(self, text):
        ...
        )  # re.DOTALL을 사용하여 줄바꿈을 포함한 모든 문자 처리
        # 각 지문에 인덱스를 붙이기
        paragraph_data = [
            {"index": i, "passage": paragraph.strip()}
    ...
    def remove_unwanted_patterns(text):
        ...
        text = re.sub(
            r"\d+ 국 어 영 역 공 통|공 통 국 어 영 역 \d+|\d+ 국 어 영 역 A 형|A 형 국 어 영 역 \d+",
            "",
            text,
        )
        return text
    ...
    def extract_and_save_to_csv(self, output_csv_path):
        ...
        for idx, (start, end) in enumerate(matching_ranges):
            ...
            problems_in_range = [
                ...
                if int(start) <= int(problem["question"].split(".")[0]) <= int(end)
            ]
...
class MilitaryKoreanAnswerExtractor:
    ...
    def add_answers_to_csv(self, csv_file, answers_dict, output_file):
        ...
        for index, row in df.iterrows():
            # id에서 문제 번호 추출
            problem_number = row["id"].split("-")[
                -1
            ]  # id에서 'generation-for-nlp-military-년도-korean-1'에서 1 추출
...
def main():
    ...
    input_folder = (
        "/data/ephemeral/home/level2-nlp-generationfornlp-nlp-04-lv3/data/military"
    )
